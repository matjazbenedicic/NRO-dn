...
#pragma omp parallel for
    ...
    {
        // Kopiraj trenutno rešitev
        vector<double> T_old = T;

        // Iteriraj po vrsticah matrike A
        for (int i = 0; i < n; i++) {
            double sum = 0.0;

            // Iteriraj po stolpcih matrike A
            for (int j = 0; j < n; j++) {
                if (i != j) {
                    sum += A[i][j] * T_old[j];  // Uporabi T_old, ne T
                }
            }

            // Izračunaj novo vrednost za i-to komponento
            T[i] = (b[i] - sum) / A[i][i];
        }
